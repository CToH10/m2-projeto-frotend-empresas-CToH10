import { companiesBySector, employedDpt, listAllDepartments, listUsers, outOfWork } from "./api.js"
import { deleteModal, editModal, newDepartment, viewDepartment } from "./modal.js"
...
export function getButtons(elem = document) {
    ...
    editBtn.forEach((btn) => {
        btn.addEventListener("click", ()=> {
            let id = btn.parentElement.parentElement.getAttribute("name")
            body.append(editModal(btn, token, id))
        })
    })  
}
